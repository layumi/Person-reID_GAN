function ap_multi = compute_AP_multiCam(good_image,junk_image1,junk_image2, index, queryCam, testCam)
good_cam = testCam(good_image);
good_cam_uni = unique(good_cam); % other camera id
ap_multi = zeros(1, 6);

% on the same camera
good_cam_now = queryCam;
ngood = length(junk_image2);
junk_image_now = [good_image junk_image1'];
good_image_now = junk_image2;
old_recall = 0; 
old_precision = 1.0; 
ap = 0; 
intersect_size = 0; 
...
for k = 1:length(good_cam_uni)
    ...
    junk_image_now = [junk_image1' junk_image2' good_image(pos_junk)];
