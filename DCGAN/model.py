...
class DCGAN(object):
    def __init__(self, sess, image_size=64, is_crop=True,
        ...
        self.d_bn1x = batch_norm(name='d_bn1x')
        self.d_bn2x = batch_norm(name='d_bn2x')
        self.d_bn3x = batch_norm(name='d_bn3x')
        self.d_bn4x = batch_norm(name='d_bn4x')
        if not self.y_dim:
            self.d_bn3 = batch_norm(name='d_bn3')
    ...
    def train(self, config):
        ...
        if config.dataset == 'mnist':
           ...
           # sample = tf.image.random_flip_left_right(sample) #fliplr
    ...
    def discriminator(self, image, y=None, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            if not self.y_dim:
                ...
                #h1x = lrelu(self.d_bn1x(conv2d(h1, self.df_dim*2, d_h=1,d_w=1, name='d_h1x_conv')))

                h2 = lrelu(self.d_bn2(conv2d(h1, self.df_dim*4, name='d_h2_conv')))
                #h2x = lrelu(self.d_bn2x(conv2d(h2, self.df_dim*4, d_h=1,d_w=1, name='d_h2x_conv')))

                h3 = lrelu(self.d_bn3(conv2d(h2, self.df_dim*8, name='d_h3_conv')))
                #h3x = lrelu(self.d_bn3x(conv2d(h3, self.df_dim*8, d_h=1,d_w=1, name='d_h3x_conv')))

                h4 = lrelu(self.d_bn4(conv2d(h3, self.df_dim*16, name='d_h4_conv')))
                #h4x = lrelu(self.d_bn4x(conv2d(h4, self.df_dim*16, d_h=1,d_w=1, name='d_h4x_conv')))
    ...
    def generator(self, z, y=None):
        with tf.variable_scope("generator") as scope:
            if not self.y_dim:
                ...
                #s4, s8, s16 ,s32 = int(s/2), int(s/4), int(s/8),int(s/16)
		...
		h4, self.h4_w, self.h4_b = deconv2d(h3,
                ...
                #add 5x5 same filter
                h5x,self.h5x_w,self.h5x_b = deconv2d(h5,
                        [self.batch_size, s, s, self.c_dim], k_h=5, k_w=5, d_h=1, d_w=1, name='g_h5x', with_w=True)

                return tf.nn.tanh(h5x)
            else:
                s = self.output_size
